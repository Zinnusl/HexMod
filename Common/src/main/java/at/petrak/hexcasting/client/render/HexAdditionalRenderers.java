...
public class HexAdditionalRenderers {
    ...
    private static void tryRenderScryingLensOverlay(GuiGraphics graphics, float partialTicks) {
        ...
        if (player.getAttributeValue(HexAttributes.holder(HexAttributes.SCRY_SIGHT)) <= 0.0 || player.getAttributeValue(HexAttributes.holder(HexAttributes.FEEBLE_MIND)) > 0)
            return;
